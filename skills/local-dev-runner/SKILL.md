...
8. For a visible-terminal fallback, print a session header before service output. Begin it with a compact printable-ASCII art logo or banner: use documented project or application branding when it is printable ASCII; otherwise render the discovered project or application name as a bounded printable-ASCII banner. Do not fetch assets, install generators, invent branding, use Unicode art, or delay launch. Keep all detailed status there: session identity, grouped commands, approved extra services, working directories, process identities, URL roles, URLs or ports, readiness state, logs, and shutdown guidance.
