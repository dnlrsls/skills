...
- Track the aggregate launcher, each service process, and any terminal host separately. An open terminal or a returned prompt is not evidence that the environment is running.
...
5. Before launch, establish one environment-session identity, aggregate and child-process ownership, a cleanup path, a readiness deadline, and the persistence mode from the user's explicit wording. A request to start or launch an environment means keep successful services running. Treat a run as temporary only when the user explicitly asks to close it after verification or use.
...
7. The launch operation performs no sleeps, readiness loops, HTTP probes, or live-log reads. Return `starting` as soon as the session and process identities are known, including any URL or port available without waiting; never keep the current agent turn open to obtain readiness. Record the aggregate PID or other identity, its exit status when available, and the first causal service failure independently of terminal-host liveness.
8. For a visible-terminal fallback, print a session header before service output. Begin it with a compact monochrome printable-ASCII terminal-dashboard banner that stylistically resembles a framed status display rather than a standalone logo: include a bounded outer frame, horizontal separators, and an aligned metadata row for discovered project, repository, and session identity. When terminal width permits, make the documented or discovered project or application name prominent in block letters; otherwise use a compact plain-text title. Place concise aligned status and service sections below the title and adapt the composition to terminal width without wrapping. Do not fetch assets, install generators, invent branding, use Unicode art, animate, or delay launch. Keep all detailed status there: session identity, grouped commands, approved extra services, working directories, process identities, URL roles, URLs or ports, readiness state, logs, and shutdown guidance.
9. If startup exits before readiness, perform at most one bounded status collection. Identify the earliest causal failure, distinguish manager-triggered or cascading shutdowns from that cause, mark the environment `failed`, and clean up owned remnants.
10. Never retry an unchanged command after a deterministic prerequisite, configuration, compatibility, permission, or platform failure. Retry only when evidence shows the blocking condition changed or an explicitly documented transient recovery authorizes it. Do not install, reconfigure, or substitute commands automatically.
11. Check readiness only through an independently running monitor that delivers completion without blocking the agent, or through a separate bounded status operation when the user requests it or later work needs the service. An independent monitor uses the repository-documented deadline or 60 seconds when none exists, never extends it silently, and stops an unusable owned startup tree on timeout.
12. Probe an endpoint that proves each service's intended role; a listening port or arbitrary HTTP response alone does not prove a usable application. Never assume conventional ports. Classify discovered URLs as user-facing application, API, asset or hot-reload server, or internal service. Do not present an asset server as the frontend application merely because it is listening; report its role and readiness separately.
